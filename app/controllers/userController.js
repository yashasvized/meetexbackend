...
let updatePassword = (req,res)=>{
    const data = jwt.verify(req.params.token,"yashasvi");
    console.log(data);
     UserModel.update({'userId':data.user }, {password:passwordLib.hashpassword(data.newpass)}).exec((err, result) => {
        if(err){
            console.log(err)
        }else{
            console.log(result);
...
let forgetPassword = (req,res)=>{
            ...
            if (req.body.email) {
                ...
                UserModel.findOne({ email: req.body.email}, (err, userDetails) => {
                    ...
                    if (err) {
                        ...
                        /* prepare the message and the api response here */
                        logger.info('User Found', 'userController: findUser()', 10)
                        userDetails.newpass = req.body.password;
                        resolve(userDetails)
                    }
                });
    ...
    .then((resolve) => {
        ...
        let apiResponse = response.generate(false, 'Password reset sent', 200, resolve)
        res.status(200)
        res.send(apiResponse)
    })
    .catch((err) => {
...
let addAMeeting = (req, res) => {
    let data = req.body;
    console.log(req.body.meetId)
    UserModel.update({userId:req.body.userId,"meeting.meetId":req.body.meetId},{$set: { "meeting.$" : req.body }})
    .exec((err, result) => {
        if (err) {
            console.log(err)
            logger.error(err.message, 'User Controller: getAllUser', 10)
            ...
            console.log(result.n + "gg")
            data.meetId = shortid.generate();
            UserModel.update({userId:req.body.userId},{$push:{meeting:data}})
            .select(' -__v -_id -password')
            .lean()
            .exec((err, result) => {
                if (err) {
...
let deleteAMeeting = (req, res) => {
    let data = req.body;
    console.log(req.body.title)
    UserModel.update({userId:req.body.userId,"meeting.meetId":req.body.meetId},{$pull: { "meeting" : {"meetId":req.body.meetId }}})
    .exec((err, result) => {
        if (err) {
            console.log(err)
            logger.error(err.message, 'User Controller: getAllUser', 10)
...
module.exports = {
    ...
    addAMeeting:addAMeeting,
    getCurrentUser:getCurrentUser,
    deleteAMeeting:deleteAMeeting
